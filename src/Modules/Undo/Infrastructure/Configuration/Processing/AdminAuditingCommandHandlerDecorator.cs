...
/// <summary>
/// Undo-private copy of the Quests B7 audit decorator template
    ...
    where T : ICommand
{
    ...
    public async Task Handle(T command, CancellationToken cancellationToken)
    {
        ...
        UndoAdminAuditing.EnqueueAudit(command, adminCommand, adminUserId, _clock, _outbox, _notificationsMapper);
    }
}
...
internal static class UndoAdminAuditing
{
    public static void EnqueueAudit(
        ...
        IDomainNotificationsMapper notificationsMapper)
    {
        var notification = new UndoAdminActionPerformedNotification(
            id: Guid.NewGuid(),
        ...
        var typeKey = notificationsMapper.GetName(typeof(UndoAdminActionPerformedNotification))
            ?? throw new InvalidOperationException(
                "UndoAdminActionPerformedNotification is not registered in DomainNotificationsMap. " +
                "Add the mapping in UndoStartup.");
