...
/// <summary>
/// Games-private copy of the Quests B7 audit decorator template
/// (Kamil decorator-per-module rule). For IAdminCommand commands:
/// requires admin authorization, then enqueues a per-module audit
/// notification in the same UoW so it commits with the command's
/// domain changes.
/// </summary>
    ...
    where T : ICommand
{
    ...
    public async Task Handle(T command, CancellationToken cancellationToken)
    {
        ...
        GamesAdminAuditing.EnqueueAudit(command, adminCommand, adminUserId, _clock, _outbox, _notificationsMapper);
    }
}
...
internal static class GamesAdminAuditing
{
    public static void EnqueueAudit(
        ...
        IDomainNotificationsMapper notificationsMapper)
    {
        var notification = new GamesAdminActionPerformedNotification(
            id: Guid.NewGuid(),
        ...
        var typeKey = notificationsMapper.GetName(typeof(GamesAdminActionPerformedNotification))
            ?? throw new InvalidOperationException(
                "GamesAdminActionPerformedNotification is not registered in DomainNotificationsMap. " +
                "Add the mapping in GamesStartup.");
