...
/// <summary>
/// Diamond-private copy of the admin audit decorator template.
/// Captures IAdminCommand executions, enforces admin authorization, and
/// enqueues an outbox audit notification.
/// </summary>
    ...
    where T : ICommand
{
    ...
    public async Task Handle(T command, CancellationToken cancellationToken)
    {
        ...
        DiamondAdminAuditing.EnqueueAudit(command, adminCommand, adminUserId, _clock, _outbox, _notificationsMapper);
    }
}
...
internal static class DiamondAdminAuditing
{
    public static void EnqueueAudit(
        ...
        IDomainNotificationsMapper notificationsMapper)
    {
        var notification = new DiamondAdminActionPerformedNotification(
            id: Guid.NewGuid(),
        ...
        var typeKey = notificationsMapper.GetName(typeof(DiamondAdminActionPerformedNotification))
            ?? throw new InvalidOperationException(
                "DiamondAdminActionPerformedNotification is not registered in DomainNotificationsMap. " +
                "Add the mapping in DiamondStartup.");
