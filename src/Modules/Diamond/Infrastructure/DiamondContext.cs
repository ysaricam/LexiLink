...
{
    public DbSet<PlayerDiamondInventory> PlayerDiamondInventories { get; set; } = null!;

    public DbSet<OutboxMessage> OutboxMessages { get; set; } = null!;

    private readonly ILoggerFactory _loggerFactory;

    public DiamondContext(DbContextOptions<DiamondContext> options, ILoggerFactory loggerFactory)
        : base(options)
    {
    ...
    protected override void ConfigureConventions(ModelConfigurationBuilder configurationBuilder)
    {
        ...
        var typedIdTypes = typeof(DiamondContext).Assembly.GetReferencedAssemblies()
            .Select(System.Reflection.Assembly.Load)
            .Concat([typeof(DiamondContext).Assembly])
