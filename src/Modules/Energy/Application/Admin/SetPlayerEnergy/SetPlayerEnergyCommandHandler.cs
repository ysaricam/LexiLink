...
internal sealed class SetPlayerEnergyCommandHandler : ICommandHandler<SetPlayerEnergyCommand>
{
    private readonly IPlayerEnergyRepository _repository;
    private readonly IClock _clock;

    internal SetPlayerEnergyCommandHandler(IPlayerEnergyRepository repository, IClock clock)
    {
        _repository = repository;
        _clock = clock;
    }

    public async Task Handle(SetPlayerEnergyCommand request, CancellationToken cancellationToken)
    {
        ...
        energy.AdminSet(request.Amount, _clock.UtcNow);
    }
}
