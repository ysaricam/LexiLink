...
{
    public DbSet<PlayerEnergy> PlayerEnergies { get; set; } = null!;

    public DbSet<OutboxMessage> OutboxMessages { get; set; } = null!;

    private readonly ILoggerFactory _loggerFactory;

    public EnergyContext(DbContextOptions<EnergyContext> options, ILoggerFactory loggerFactory)
        : base(options)
    {
    ...
    protected override void ConfigureConventions(ModelConfigurationBuilder configurationBuilder)
    {
        ...
        var typedIdTypes = typeof(EnergyContext).Assembly.GetReferencedAssemblies()
            .Select(System.Reflection.Assembly.Load)
            .Concat([typeof(EnergyContext).Assembly])
