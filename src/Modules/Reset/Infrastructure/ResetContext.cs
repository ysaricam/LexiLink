...
{
    public DbSet<PlayerResetInventory> PlayerResetInventories { get; set; } = null!;

    public DbSet<OutboxMessage> OutboxMessages { get; set; } = null!;

    private readonly ILoggerFactory _loggerFactory;

    public ResetContext(DbContextOptions<ResetContext> options, ILoggerFactory loggerFactory)
        : base(options)
    {
    ...
    protected override void ConfigureConventions(ModelConfigurationBuilder configurationBuilder)
    {
        ...
        var typedIdTypes = typeof(ResetContext).Assembly.GetReferencedAssemblies()
            .Select(System.Reflection.Assembly.Load)
            .Concat([typeof(ResetContext).Assembly])
