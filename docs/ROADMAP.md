...
| **GO6** | ✅ Done | CI/CD. GitHub Actions manual/tag workflow builds and pushes `ghcr.io/ysaricam/lexilink:<git-sha>` + `latest`, SSHes to the VPS, checks out the exact commit, runs `LEXILINK_IMAGE=<sha-image> ./scripts/deploy.sh`, and verifies readiness. `deploy.sh` supports both local-build and GHCR-pull deploy modes. GitHub secrets configured and first production workflow deploy completed successfully. |
