...
| **GO5** | ✅ Done | Backups + ops hardening. Repo-side artifacts prepared: `scripts/backup-db.sh`, `scripts/restore-db.sh`, `docs/DEPLOYMENT.md`, and compose resource/log limits. Server-side manual backup, offsite copy to operator PC, restore drill, nightly cron, `ufw`, SSH hardening, and final health check all completed. |
| **GO6** | 🔵 In progress | CI/CD. Repo-side prepared: GitHub Actions manual/tag workflow builds and pushes `ghcr.io/ysaricam/lexilink:<git-sha>` + `latest`, SSHes to the VPS, checks out the exact commit, runs `LEXILINK_IMAGE=<sha-image> ./scripts/deploy.sh`, and verifies readiness. `deploy.sh` now supports both local-build and GHCR-pull deploy modes. Remaining: configure GitHub secrets and run the first production workflow deploy. |
