...
`StepBudget`, a `GameState`, and plain Hint/Undo/Reset usage counters.
...
A single quest assignment to a single player. Aggregate root, identified by `PlayerQuestId` (its own Guid, unrelated to `PlayerId`). Holds `_playerId`, `_questDefinitionId`, `_progressBaselineSnapshot`, `_state`, `_issuedAt`, `_claimedAt`, and `_expiresAt`. Post Sprint Q1 progress is **computed at read time** from Stats counters; the aggregate doesn't persist `Progress`/`Goal`/rewards — those live on the linked `QuestDefinition`. Lifecycle: `IssueFor(playerId, questDefinitionId, baselineSnapshot, issuedAt, expiresAt?)` (creates in `Active` state). Behavior (Sprint H + UR + D reshape): `Claim(now, isReadyToClaim, energyReward, hintReward, undoReward, resetReward, diamondReward)` — the handler passes `isReadyToClaim` (computed from current Stats counter minus baseline ≥ definition's threshold, and not past `_expiresAt`) and the catalog's 5-reward tuple; aggregate flips `Active → Claimed` and emits `PlayerQuestClaimedDomainEvent` carrying all five rewards so Energy, Hint, Undo, Reset, and Diamond outbox consumers can each grant their bonus event-driven (each consumer guards on its reward's positivity). Expired daily rows are DELETEd by `GetActiveQuestsQueryHandler` on the next sync, not transitioned. Emits `PlayerQuestIssuedDomainEvent`, `PlayerQuestClaimedDomainEvent`.

### QuestDefinition — `Modules/Quests/Domain/PlayerQuests/QuestDefinition.cs`
Catalog entry describing how a quest is issued and rewarded. Aggregate root identified by `QuestDefinitionId`. Holds `_name` (≤64 chars), `_description` (≤256 chars), `_trigger`, `_threshold`, `_energyReward`, `_hintReward`, `_undoReward`, `_resetReward`, `_diamondReward`, `_prerequisiteQuestDefinitionId?` (FK to another definition or null), `_progressBaseline`, `_isActive`. Sprint H split the single `_reward` into `(EnergyReward, HintReward)`; Sprint UR expanded the pair to 4 fields; Sprint D expanded it to **5 fields** (`+ DiamondReward`) so a quest can deliver any combination. `QuestRewardMustHaveAtLeastOnePositiveRule` enforces ≥1 positive across all 5. Lifecycle: `Create(name, description, trigger, threshold, energyReward, hintReward, undoReward, resetReward, diamondReward, prereqId?, progressBaseline, prerequisiteWouldCreateCycle)` — handler walks the prereq chain ahead of time and passes the boolean. `Update(description, threshold, energyReward, hintReward, undoReward, resetReward, diamondReward, prereqId?, progressBaseline, prerequisiteWouldCreateCycle)` — Name and Trigger are immutable post-create (changing them would re-key PlayerQuest history). `Deactivate` / `Reactivate` idempotent; deactivated definitions disappear from `/quests/me` listings (claim history rows stay intact). Emits `QuestDefinitionCreatedDomainEvent`, `QuestDefinitionUpdatedDomainEvent`, `QuestDefinitionActivationChangedDomainEvent`.
...
`HintAllowance` remains an immutable VO with `Of(int total)`, `Remaining`, `Used`, and `Consume()`; Game uses field reassignment: `_hintAllowance = _hintAllowance.Consume();`. Sprint D post-UR1 model sets `IGameConfigurationService.ResolveHints` to 0, so `HasFreeHintRemaining` is false for new games and every `UseHintCommand` goes through `IHintGuard` before Game mutates. `Game.HintsUsed` (in `GameDetailsDto`) remains the legacy/free-quota counter and now stays 0 for inventory-backed hints. Sprint UR1 removed `UndoAllowance` and `ResetAllowance`; Undo/Reset keep only `_undosUsed` + `_resetsUsed` counters in Game, while persistent inventories live in Undo and Reset modules.
...
- `PlayerQuestClaimedDomainEvent` — emitted by `Claim(now, isReady, energyReward, hintReward, undoReward, resetReward, diamondReward)`; carries `PlayerQuestId`, `PlayerId`, `QuestDefinitionId`, and the 5 reward fields. Mapped to `QuestClaimedIntegrationEvent` via outbox so Energy, Hint, Undo, Reset, and Diamond can each grant their bonus event-driven (each consumer guards on its reward's positivity).
- `QuestDefinitionCreatedDomainEvent` — emitted by `Create(...)`; carries `QuestDefinitionId`, `Name`, `Trigger` (string), `Threshold`, `EnergyReward`, `HintReward`, `UndoReward`, `ResetReward`, `DiamondReward`, `PrerequisiteQuestDefinitionId?`, `ProgressBaseline` (string).
...
### PlayerDiamondInventory (5) — `Diamond/Domain/PlayerDiamondInventories/Events/`
- `PlayerDiamondInventoryInitializedDomainEvent` — emitted by `InitializeFor(playerId, initialBalance)`; carries `PlayerId`, `InitialBalance`.
- `PlayerDiamondConsumedDomainEvent` — emitted by `Consume(amount, now)`; carries `PlayerId`, `Amount`, `RemainingBalance`, `ConsumedOn`.
- `PlayerDiamondGrantedDomainEvent` — emitted by `GrantBonus(amount, now)`; carries `PlayerId`, `Amount`, `NewBalance`, `GrantedOn`.
- `PlayerDiamondAdminSetDomainEvent` — emitted by `AdminSet(newBalance, now)`; carries `PlayerId`, `NewBalance`, `SetOn`.
- `PlayerDiamondAdminResetDomainEvent` — emitted by `AdminReset(now)`; carries `PlayerId`, `ResetOn`.
...
- `EnergyAmountCannotExceedMaximumRule` — invariant guard for direct amount-setting paths.
...
- `QuestMustBeReadyToBeClaimedRule` — `Claim(now, isReadyToClaim, energyReward, hintReward, undoReward, resetReward, diamondReward)` requires `_state == Active && isReadyToClaim` (caller-supplied flag computed from Stats counter vs threshold + not past expiry).
- `QuestThresholdMustBePositiveRule` — `QuestDefinition.Create` / `Update` require `threshold > 0`.
- `QuestRewardMustHaveAtLeastOnePositiveRule` — Sprint H introduced this rule with 2 reward fields (Energy + Hint); Sprint UR5 widened it to 4; Sprint D widened it to **all 5** (`EnergyReward, HintReward, UndoReward, ResetReward, DiamondReward`). `Create` / `Update` require each ≥ 0 **and** at least one > 0. Empty-reward quests would be inert and are rejected.
...
`IDiamondConfigurationService` exposes `InitialBalance` for fresh
Diamond inventory initialization. Infrastructure reads
`Diamond:InitialBalance`; the default is 0 so new players earn Diamond
through quests/admin grants until Shop/IAP flows exist.
