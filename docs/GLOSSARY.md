...
A single quest assignment to a single player. Aggregate root, identified by `PlayerQuestId` (its own Guid, unrelated to `PlayerId`). Holds `_playerId`, `_questDefinitionId`, `_progressBaselineSnapshot`, `_state`, `_issuedAt`, `_claimedAt`, and `_expiresAt`. Post Sprint Q1 progress is **computed at read time** from Stats counters; the aggregate doesn't persist `Progress`/`Goal`/rewards — those live on the linked `QuestDefinition`. Lifecycle: `IssueFor(playerId, questDefinitionId, baselineSnapshot, issuedAt, expiresAt?)` (creates in `Active` state). Behavior (Sprint H + UR reshape): `Claim(now, isReadyToClaim, energyReward, hintReward, undoReward, resetReward)` — the handler passes `isReadyToClaim` (computed from current Stats counter minus baseline ≥ definition's threshold, and not past `_expiresAt`) and the catalog's 4-reward tuple; aggregate flips `Active → Claimed` and emits `PlayerQuestClaimedDomainEvent` carrying all four rewards so Energy, Hint, Undo, and Reset outbox consumers can each grant their bonus event-driven (each consumer guards on its reward's positivity). Expired daily rows are DELETEd by `GetActiveQuestsQueryHandler` on the next sync, not transitioned. Emits `PlayerQuestIssuedDomainEvent`, `PlayerQuestClaimedDomainEvent`.

### QuestDefinition — `Modules/Quests/Domain/PlayerQuests/QuestDefinition.cs`
Catalog entry describing how a quest is issued and rewarded. Aggregate root identified by `QuestDefinitionId`. Holds `_name` (≤64 chars), `_description` (≤256 chars), `_trigger`, `_threshold`, `_energyReward`, `_hintReward`, `_undoReward`, `_resetReward`, `_prerequisiteQuestDefinitionId?` (FK to another definition or null), `_progressBaseline`, `_isActive`. Sprint H split the single `_reward` into `(EnergyReward, HintReward)`; Sprint UR expanded the pair to **4 fields** (`+ UndoReward, ResetReward`) so a quest can deliver any combination. `QuestRewardMustHaveAtLeastOnePositiveRule` enforces ≥1 positive across all 4. Lifecycle: `Create(name, description, trigger, threshold, energyReward, hintReward, undoReward, resetReward, prereqId?, progressBaseline, prerequisiteWouldCreateCycle)` — handler walks the prereq chain ahead of time and passes the boolean. `Update(description, threshold, energyReward, hintReward, undoReward, resetReward, prereqId?, progressBaseline, prerequisiteWouldCreateCycle)` — Name and Trigger are immutable post-create (changing them would re-key PlayerQuest history). `Deactivate` / `Reactivate` idempotent; deactivated definitions disappear from `/quests/me` listings (claim history rows stay intact). Emits `QuestDefinitionCreatedDomainEvent`, `QuestDefinitionUpdatedDomainEvent`, `QuestDefinitionActivationChangedDomainEvent`.
...
- `PlayerQuestClaimedDomainEvent` — emitted by `Claim(now, isReady, energyReward, hintReward, undoReward, resetReward)`; carries `PlayerQuestId`, `PlayerId`, `QuestDefinitionId`, and the 4 reward fields. Mapped to `QuestClaimedIntegrationEvent` via outbox so Energy, Hint, Undo, and Reset can each grant their bonus event-driven (each consumer guards on its reward's positivity).
- `QuestDefinitionCreatedDomainEvent` — emitted by `Create(...)`; carries `QuestDefinitionId`, `Name`, `Trigger` (string), `Threshold`, `EnergyReward`, `HintReward`, `UndoReward`, `ResetReward`, `PrerequisiteQuestDefinitionId?`, `ProgressBaseline` (string).
...
- `QuestMustBeReadyToBeClaimedRule` — `Claim(now, isReadyToClaim, energyReward, hintReward, undoReward, resetReward)` requires `_state == Active && isReadyToClaim` (caller-supplied flag computed from Stats counter vs threshold + not past expiry).
- `QuestThresholdMustBePositiveRule` — `QuestDefinition.Create` / `Update` require `threshold > 0`.
- `QuestRewardMustHaveAtLeastOnePositiveRule` — Sprint H introduced this rule with 2 reward fields (Energy + Hint); Sprint UR5 widened it to **all 4** (`EnergyReward, HintReward, UndoReward, ResetReward`). `Create` / `Update` require each ≥ 0 **and** at least one > 0. Empty-reward quests would be inert and are rejected.
...
Public contract emitted by Quests' outbox after `PlayerQuestClaimedDomainEvent` lands. Sprint H added the `(EnergyReward, HintReward)` pair; Sprint UR widened the payload to all four reward fields (`EnergyReward, HintReward, UndoReward, ResetReward`) plus `PlayerId`, `PlayerQuestId`, `QuestDefinitionId`. Consumed by **four** independent handlers — Energy.Application, Hint.Application, Undo.Application, and Reset.Application each implement `QuestClaimedIntegrationEventHandler` and skip when their own reward field == 0, otherwise dispatching the matching `Grant*Command`. This is LexiLink's reverse cross-module event pattern: four modules carry granular ArchTest allows on `Quests.IntegrationEvents`. Quests.Domain/Application/Infrastructure remain forbidden from any consumer module.
