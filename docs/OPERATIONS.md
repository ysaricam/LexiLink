...
| `GET /admin/audit` | `AuthenticatedAdmin` | Paged list of admin actions from `administration.AdminActionAudit`, newest first. Optional filters: `adminUserId`, `targetType`, `targetId`. Pagination via `offset` / `limit` (default 50, max 200). Each row carries actor, action type, target type/id, opaque JSON payload, and OccurredOn. Populated by the per-module admin auditing decorator that B7–B10 wire into each consumer module (B5 only ships the projection; rows currently appear only when a producer module publishes `AdminActionPerformedIntegrationEvent`). |
