...
| `POST /admin/players/{playerId:guid}/energy/grant` | `AuthenticatedAdmin` | Grant bonus energy (`{ amount }`, > 0). Wraps the internal `GrantEnergyCommand`; applied amount is capped at the player's maximum energy. |
